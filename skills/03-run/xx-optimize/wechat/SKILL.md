---
name: xx-optimize
description: 性能优化技能。当产品在低端机卡顿、AI 接口等待白屏、历史列表滚动掉帧、或需要优化帧循环计算、运动交互时使用。覆盖 AI 产品典型性能场景（流式渲染、长等待、长列表、冷启动、节流防抖）和零体验降级原则，按P0/P1/P2/P3分级优化，输出可直接应用的参考实现。
---

# 性能优化与运动交互

> 零体验降级。AI 产品先治"等接口"和"长列表"，再治计算，最后才考虑降级体验。
> 主要给 AI 执行优化策略，给人判断优化是否到位、是否过度降级。
...
| 级别 | 措施 | 是否需确认 | 示例 |
|------|------|-----------|------|
| **P0** | 纯计算优化 / AI 等待优化 | 无需确认 | 查表、缓存、流式渲染、骨架屏、请求并行 |
| **P1** | 采样频率调整 | 无需确认 | 降低非关键模块刷新率、加速度计降频 |
| **P2** | 视觉降级 | 需产品确认 | 移除 blur、降 Canvas 分辨率、减粒子 |
| **P3** | 交互变更 | 必须确认 | 改变核心逻辑、减少动画、改交互方式 |
...
> AI 产品的性能瓶颈通常在 AI 接口等待和长列表渲染，不在纯计算。本章覆盖 5 个最常见场景，每个给"判断标准 + AI 参考实现"。
...
**判断标准：** AI 性能场景 6 项 + 真机测试 5 项为硬门槛，任一未通过不发版。
