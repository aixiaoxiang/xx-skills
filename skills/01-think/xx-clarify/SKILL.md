---
name: xx-clarify
description: 需求澄清技能。当用户有模糊的产品想法但说不清到底要解决什么问题、给谁用、做成什么样时使用。默认快速模式一次走完 5 步（锁定问题陈述 / JTBD 重定义 / 用户故事映射找盲点 / Kano 分类与优先级 / 输出澄清卡）直接输出一张简化澄清卡；用户说"再深一点"才进入该步的深入模式，展开 5W1H 全表、Forces of Progress 四股力、walking skeleton 断点表、Kano 五类、OST 结构图等方法论细节。每步附判断标准与 AI 执行约束。
---
...
帮你在**快速模式约 15 分钟 / 深入模式 30-60 分钟**内，把"我想做个 XX 工具"这种模糊想法，澄清成**结构化的问题定义 + 机会清单**，输出一张**澄清卡**。
...
- **快速模式（默认）：** AI 一次走完 5 步，直接输出**简化澄清卡**（每步只取核心结论，不展开方法论细节），不每步停下来等确认。
- **深入模式（按需）：** 用户说"再深一点"/"第 N 步我不确定"/"展开说说"时，才进入**该步**的深入模式（展开 5W1H 全表、Forces of Progress 四股力、walking skeleton 断点表、Kano 五类、OST 结构图）。深入模式按原有"每步确认"节奏走。
...
- [ ] 一条 walking skeleton（端到端）+ 至少 2 个标了"卡住后果"的断点
- [ ] MVP 取法明确（哪些进 MVP，兴奋型不超过 1 个）
- [ ] 至少 2 条可证伪假设 + 指向具体 skill 的下一步
...
> 以下是**深入模式**的完整演示（5 步全展开）。快速模式输出会更简——直接给一张简化澄清卡，不展开 5W1H 全表、四股力、断点表、Kano 表。
...
把你的模糊想法告诉我，我默认**快速模式**一次走完 5 步，直接给你一张简化澄清卡。你看完觉得哪一步想再深一点（比如"第 3 步断点我不确定"/"展开说说 Kano"），就说"再深一点"或"第 N 步深入"，我进入那一步的深入模式，按"每步确认"的节奏跟你走。你只需用自然语言说想法和判断"对/不对/改哪里"。

**对话示例：**
- "我想做个取色工具" → 我快速走完 5 步，直接给简化澄清卡，再问"要不要对某一步深入"
- "第 2 步的 outcome 我觉得不对" → 进入第 2 步深入模式，原地改，不跳步
- "断点我觉得还少一个" → 进入第 3 步深入模式补断点
- "再深一点" → 我问你指哪一步，进入该步深入模式
- "5 步都走完了，可以了" → 我把澄清卡给你，建议你走 xx-research 验证最危险假设
